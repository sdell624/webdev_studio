...
const updatePage = async () => {
  ...
  const sugarOver15 = fruitsArray.filter((item) => {
    return item.nutritions.sugar > 15;
  })

  console.log(sugarOver15);

  const fruitNames = sugarOver15.map((item) => {
    return item.name;
  })
  ...
  const myElt = document.createElement("p");
  var text = "Here are the fruits with over 15 grams of sugar per 100 grams: ";
  for (var i = 0; i < sugarOver15.length; i++) {
    
    text += sugarOver15[i].name + " (" + sugarOver15[i].nutritions.sugar + " grams)";
    if (i != sugarOver15.length) {
      text += ", ";
    }
  }
  const mText = document.createTextNode(text);
  myElt.append(mText);

  // TODO: Append your new element to the page
  document.getElementById("cs1300-gallery").append(myElt);
}
